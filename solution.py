...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    layers.append(nn.Linear(sizes[len(sizes) - 2], sizes[len(sizes) - 1]))
...
class Actor(nn.Module):
    ...
    def _log_prob_from_distribution(self, pi, act):
        ...
        """
        return pi.log_prob(act) #Why, tho?
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        self.ret_buf[path_slice] = discount_cumsum(rews, self.lam)[:-1]
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            distribution = self.actor.forward(state)[0]
            action = distribution.sample()
            v = self.critic.forward(state)
            logp = self.actor._log_prob_from_distribution(distribution, action)
        
        return action.cpu().detach().numpy(), v.cpu().detach().numpy(), logp.cpu().detach().numpy()
    ...
    def get_action(self, obs):
        ...
        distribution = self.actor.forward(obs)[0]
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        #Approximation of the gradient of the expected rewards
        loss = (-data['tdres']*agent.actor.forward(data['obs'], data['act'])[1]).mean()
        ...
        for _ in range(100):
            ...
            val = agent.critic.forward(data['obs'])
            l = nn.MSELoss(reduction = 'sum')
            #The "difference" between our prediction and the approximation (not the real groundtruth)
            loss = l(data['ret'], val)
